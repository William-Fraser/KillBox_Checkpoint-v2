...
public class PlayerController : MonoBehaviour
{
    ...
    public float _deathDistancetoB = 100f; // death occurs after distance traveled
    public float _deadlyVelocity = 50f; // death occurs after velocity reached
    public bool _useVelocityDeath;
    ...
    void Start()
    {
        ...
        r_fog = RespawnFog.GetComponent<Image>();
        r_respawning = true;
        r_respawnOnce = false;
    ...
    private void OnCollisionStay(Collision collision)
    {
        ...
        { 
            ...
            p_velocity = 1; // no longer gaining velocity
        }
    }
    ...
    private void Fall()
    {
        ...
        p_velocity = p_velocity * (accelerationBoost + 9.84f); // magic number is the weight of gravity ()
        InstKillBarrier();   
    }
    private void InstKillBarrier()
    {
        if (p_canInstDB == true)
        {
            ...
            p_canInstDB = false;
            p_deathBarrier = transform.position.y - _deathDistancetoB;
        }
    }
    private void ActivateKillBarrier()
    {
        if (transform.position.y <= p_deathBarrier && !_useVelocityDeath )
        {
            Respawn();
        }
    }
    private void VelocityDeath()
    {
        if (p_velocity == _deadlyVelocity && _useVelocityDeath)
        {
            Respawn();
        }
    }
